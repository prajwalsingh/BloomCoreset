...
if __name__ == "__main__":
    ...
    os.makedirs('data/AffinityMat_sum/{}/'.format(downstream_data_name), exist_ok=True)
    ...
    plt.tight_layout()
    plt.savefig('data/AffinityMat_sum/{}/downstream_{}_codelength_{}_hash_bloomsamples_{}.pdf'.format(downstream_data_name, downstream_data_name, code_length, openset_name), bbox_inches='tight', dpi=300)
    plt.clf()
    plt.close('all')
    #####
    ...
    start_time = time()
    simscore = torch.mm(torch.from_numpy(downstream_code).to('cuda'), torch.from_numpy(openset_code_filter).to('cuda').T)
    # simscore_temp = torch.sum(torch.where(simscore>tau, 1, 0), dim=0)
    simscore_temp = torch.sum(simscore, dim=0)
    simscore_idx  = torch.argsort(simscore_temp, dim=-1, descending=True, stable=False).cpu().numpy()[:filter_openset_size]
    ...
    plt.yticks(fontsize=18)
    plt.tight_layout()
    plt.savefig('data/AffinityMat_sum/{}/downstream_{}_codelength_{}_hash_bloomfilter_{}.pdf'.format(downstream_data_name, downstream_data_name, code_length, openset_name), bbox_inches='tight', dpi=300)
    plt.clf()
    plt.close('all')
    #####
    ...
    with open('data/Filter/{}/downopen_sum_{}_subset_codelength_{}_hash_k{}_{}_kmeans.txt'.format(downstream_data_name, openset_name ,code_length, n_clusters, downstream_data_name), 'w') as file:
    ...
    np.save('data/Filter/{}/downopen_sum_ssl_filter_counting_{}_{}_fast_k{}_with_freq_clip_codelength_{}_{}_kmeans.npy'.format(downstream_data_name, openset_name, downstream_data_name, n_clusters, code_length, openset_name), path_with_freq)
