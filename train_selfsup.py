...
from sslmethods import get_method_class
...
def parse_args():
    ...
    parser.add_argument('--data_path_file', type=str, default='./data/downstream_ssl_filterdata.npy')
    ...
    parser.add_argument('--data_folder2', type=str, default=None, 
                        help='dataset2 path')
    parser.add_argument('--with_freq', type=str, default=False, 
                        help='dataset contains images with frequency')
    parser.add_argument('--use_freq', type=str, default=False, 
                        help='using frequency while training')
    ...
    parser.add_argument('--balance_fact', 
                        type=float, default=0.9, help='Balance factor term beta for frequency based loss')
    ...
    if args.use_freq:
        args.model_name = '{}_{}_pretrain_{}_clip'.format(args.dataset, args.model, args.method)
    else:
...
def get_dataset(args, dataset, data_folder, data_path_file, val=False):
    ...
    if dataset.startswith('imagenet_sub'):
        ...
        #                                      transform=train_transform)
        train_dataset = ImageList(np.load(data_path_file, allow_pickle=False), with_freq=args.with_freq, transform=train_transform)
    else:
        raise NotImplementedError
...
def set_loader(args):
    ...
    if args.merge_dataset:
        ...
        train_dataset = get_dataset(args, args.dataset, args.data_folder, args.data_path_file)
    
    drop_last = True if args.method in ['moco', 'swav'] else False
    train_loader = torch.utils.data.DataLoader(
            train_dataset, batch_size=args.batch_size, shuffle=True,
            num_workers=args.num_workers, pin_memory=False, drop_last=drop_last)
...
def train(train_loader, model, optimizer, epoch, args, scaler=None):
    ...
    for idx, data in enumerate(train_loader):
        ...
        if args.with_freq:
            images, images_freq = data[0], data[1].view(-1, 1)
        else:
            images = data
        ...
        if type(images) != list:  # MAE method uses a single-viewed batch
            ...
            if args.use_freq:
                images_freq = images_freq.cuda(non_blocking=True)
                weights = (1 - args.balance_fact) / (1 - (args.balance_fact**images_freq))
            bsz = img1.shape[0]
        ...
        if args.precision:
            with torch.cuda.amp.autocast():
                if args.method == 'simclr':
                    loss = model.compute_ssl_loss(img1, img2, images_weights=weights)
                else:
