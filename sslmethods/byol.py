...
class BYOL(BaseSelfSupervisedModel):
    def __init__(self, backbone: nn.Module, params: Namespace, use_momentum=True):
        ...
        self.online_net = nn.Sequential(self.online_encoder, self.online_predictor)
    ...
    def compute_ssl_loss(self, x1, x2=None, return_features=False):
        ...
        online_pred = self.online_net(x)
    
        with torch.no_grad():
            target_encoder = self._get_target_encoder() if self.use_momentum else self.online_encoder
            target_proj = target_encoder(x)
            target_proj.detach_()

        online_pred_one, online_pred_two = online_pred[:batch_size], online_pred[batch_size:]
        target_proj_one, target_proj_two = target_proj[:batch_size], target_proj[batch_size:]
...
#         else:
    ...
    def on_epoch_end(self, cur_epoch, tot_epoch):
        if self.ema_scheduler:
            ...
            print('EMA beta: {}'.format(new_beta))